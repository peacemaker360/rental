...
class FrontendI18nTests(unittest.TestCase):
    ...
    def test_unauthenticated_start_screen_is_localized(self):
        ...
        self.assertIn("reconcileDetailSelection();\n  render();\n  startMetadataPolling();", self.app_js)
        ...
        self.assertIn('const showAccessRequestForm = hasLikelySignInToken() || Boolean(request);', self.app_js)
        self.assertIn('step.action.type === "request_access" ? "data-auth-request-jump" : "data-auth-retry"', self.app_js)
        self.assertIn('class="primary-button auth-step-action"', self.app_js)
        self.assertIn('${showAccessRequestForm ? `<div id="accessRequestSection" class="auth-start-actions">', self.app_js)
    ...
    def test_state_aware_landing_persists_access_requests(self):
        ...
        self.assertIn('if (["pending_request", "no_profile", "access_denied"].includes(state.authReason)) return state.authReason;', self.app_js)
        self.assertIn('state.authStatus === "signed_out" && ["pending_request", "no_profile", "access_denied"].includes(state.authReason)', self.app_js)
        self.assertIn('errorCode === "ACCESS_REQUEST_PENDING"', self.app_js)
        self.assertIn('errorCode === "ACCESS_PROFILE_NOT_FOUND"', self.app_js)
        self.assertIn("error?.data?.accessRequest", self.app_js)
        self.assertIn("saveStoredAccessRequest(state.accessRequestResult);", self.app_js)
        self.assertIn('authState === "pending_request"', self.app_js)
        self.assertIn('authState === "access_denied"', self.app_js)
        self.assertIn('{type: "retry_access", label: t("actions.retry_access")}', self.app_js)
        self.assertIn('{type: "request_access", label: t("actions.request_access")}', self.app_js)
        self.assertIn("target.dataset.authRequestJump", self.app_js)
    ...
    def test_messages_auto_dismiss_late_and_can_be_closed(self):
        ...
        self.assertIn('id="messageAction"', self.index_html)
        self.assertIn('id="messageClose"', self.index_html)
        self.assertIn("window.setTimeout(clearMessage, 60000)", self.app_js)
        self.assertIn("messageClose?.addEventListener(\"click\"", self.app_js)
        self.assertIn("messageAction?.addEventListener(\"click\"", self.app_js)
        self.assertIn(".message-action", self.styles_css)
        self.assertIn(".message-close", self.styles_css)
        self.assertIn(".message[hidden]", self.styles_css)

    def test_metadata_poller_only_offers_an_explicit_refresh(self):
        self.assertIn("const metadataPollIntervalMs = 30000;", self.app_js)
        self.assertIn("const metadataPollLifetimeMs = 10 * 60 * 1000;", self.app_js)
        self.assertIn("function startMetadataPolling()", self.app_js)
        self.assertIn("async function pollTenantMetadata()", self.app_js)
        self.assertIn('const result = await api("/meta");', self.app_js)
        self.assertIn('showMessage(t("messages.newer_data_available"), false, {', self.app_js)
        self.assertIn("onClick: refreshFromMetadataPrompt", self.app_js)
        poller = self.app_js[self.app_js.index("async function pollTenantMetadata()"):self.app_js.index("function startMetadataPolling()")]
        self.assertNotIn("await loadData()", poller)
        self.assertIn('window.addEventListener("pagehide", stopMetadataPolling);', self.app_js)
